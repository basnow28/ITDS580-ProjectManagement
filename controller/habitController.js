...
exports.respondToHabitInvite = async (req, res, next) => {
  try {
    const { habitId } = req.params;
    const userId = req.userId;
    ...
    const index = habit.pendingInvites.indexOf(userId);
    if (index === -1) {
      return res.status(400).json({ message: "You were not invited to this habit" });
    }

    habit.pendingInvites.splice(index, 1);
    ...
    res.status(200).json({ message: `You have ${accept ? "joined" : "declined"} the habit.` });
  } catch (error) {
    next(error);
  }
};